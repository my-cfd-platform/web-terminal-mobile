const validationInputTexts = {
    EMAIL: 'Enter a valid email',
    REPEAT_PASSWORD: 'Repeat password here',
    REPEAT_PASSWORD_MATCH: 'Passwords must match',
    REQUIRED_FIELD: 'Required any value',
    USER_AGREEMENT: 'Please accept the Terms of service and Privacy Policy to continue',
    PASSWORD_MIN_CHARACTERS: 'Min 8 characters',
    PASSWORD_MAX_CHARACTERS: 'Max 31 symbols',
    PASSWORD_MAX_MIN_CHARACTERS: 'Your new password must be between 8 and 31 characters',
    PASSWORD_MAX_MIN_OLD_CHARACTERS: 'Your password must be between 8 and 31 characters',
    PASSWORD_MATCH: 'Min one number and one letter',
    NEW_PASSWORD: 'Password must be at least 8 characters long',
    CONFIRMATION_PASSWORD: 'New password and confirmation don\'t match'
}

Object.freeze(validationInputTexts)
export default validationInputTexts